...
namespace EatList_SPA.Controllers
{
    public class IdentityController : Controller
    {
        ...
        private async Task<HttpResponseMessage> ApiCallUsingUserAccessToken()
        {
            ...
            return await client.GetAsync("http://localhost:60596/api/identity");
        }

        private async Task<HttpResponseMessage> ApiCallUsingClientCredentials()
        {
            var tokenClient = new TokenClient("http://localhost:5100/connect/token", "mvcex", "secret");
